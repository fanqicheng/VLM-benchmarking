...
def _ensure_timm_ctp():
    ...
    if os.path.exists(timm_ctp_tar):
        ...
        print("[MI-Zero] timm_ctp.tar not found, skipping (may cause errors)")
...
def load_mi_zero(
    ...
    if ckpt_path is None:
        raise ValueError(
            "\n[MI-Zero] no hugging face, manual download needed\n"
        )
...
def load_quiltnet(device: str = "cuda:0") -> FMWrapper:
    _ensure_packages(open_clip="open_clip_torch")
    model, _, preprocess_fn = open_clip.create_model_and_transforms('hf-hub:wisdomik/QuiltNet-B-16')
    ...
    def _encode_image(img_batch: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            return F.normalize(model.encode_image(img_batch), p=2, dim=-1)

    def _encode_text(texts: List[str]) -> torch.Tensor:
        with torch.no_grad():
            tokens = tokenizer(texts).to(device)
            return F.normalize(model.encode_text(tokens), p=2, dim=-1)

    return FMWrapper(
        name="quiltnet", model=model, preprocess=_preprocess,
        has_text_encoder=True, encode_image=_encode_image,
        encode_text=_encode_text, image_size=224,
    )
