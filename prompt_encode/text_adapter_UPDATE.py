...
def _move_batch_to_device(batch, device: str):
    if isinstance(batch, dict):
        ...
        for k, v in batch.items():
            if isinstance(v, list):
                try:
                    result[k] = torch.tensor(v).to(device)
                except (ValueError, RuntimeError):
                    max_len = max(len(x) for x in v)
                    padded = [x + [0] * (max_len - len(x)) for x in v]
                    result[k] = torch.tensor(padded).to(device)
            elif hasattr(v, "to"):
    ...
    if hasattr(batch, "to"):
        try:
            ...
            return _move_batch_to_device(dict(batch), device)
    return batch
    ...
    def encode_text(texts: List[str]) -> torch.Tensor:
        ...
        for chunk in _batched(texts, batch_size=batch_size):
            ...
            if isinstance(tokens, list):
                ...
                tokens = {k: torch.tensor(v).to(device) if isinstance(v, list) else v.to(device) if hasattr(v, 'to') else v for k, v in tokens.items()}
            else:
...
def attach_text_adapter(
    ...
    *,
    ...
    if name == "plip":
        ...
        fm.encode_text = build_hf_forward_text_adapter(
            fm.model, tokenizer, device, batch_size=batch_size,  max_length=77, prefer_get_text_features=True
        )
        fm.has_text_encoder = True
        return

    if name in {"pathgen-clip", "patho-clip"}:
        if tokenizer is None:
            raise RuntimeError(f"{fm.name}: tokenizer must be provided from loader.")
        fm.encode_text = build_openclip_text_adapter(
            fm.model, tokenizer, device, batch_size=batch_size
